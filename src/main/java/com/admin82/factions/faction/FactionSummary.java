 ...
 */
public record FactionSummary(UUID id, String name, int memberCount, int onlineCount, long totalWealth) {
    ...
    public static FactionSummary fromNetwork(FriendlyByteBuf buf) {
        return new FactionSummary(buf.readUUID(), buf.readUtf(64), buf.readVarInt(), buf.readVarInt(), buf.readLong());
    }
}
